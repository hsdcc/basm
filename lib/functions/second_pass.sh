...
second_pass() {
  ...
  for line in "${ins_array[@]}"; do
    ...
    if [[ "$line" =~ $movss_rr_pattern ]]; then
      ...
      reg="${BASH_REMATCH[1]}"
      xmm="${BASH_REMATCH[2]}"
      mod_rm=$(build_mod_rm 3 "${regs[$reg]}" "${xmm_regs[$xmm]}")
    ...
    elif [[ "$line" =~ ^(movzx|movsx)[[:space:]]+([er][a-z]{2}|r[89]|r1[0-5]),[[:space:]]+([ab][lh]|[cd][lh]|dil|sil|bpl|spl|r[89]b|r1[0-5]b|[er][a-z]{2}|r[89]|r1[0-5])$ ]]; then
      op="${BASH_REMATCH[1]}"
      dst="${BASH_REMATCH[2]}"
      src="${BASH_REMATCH[3]}"
      dst_reg=$(get_reg_num "$dst")
    ...
    elif [[ "$line" =~ ^movsxd[[:space:]]+([er][a-z]{2}|r[89]|r1[0-5]),[[:space:]]+([er][a-z]{2}|r[89]|r1[0-5])$ ]]; then
      dst="${BASH_REMATCH[1]}"
      src="${BASH_REMATCH[2]}"
      dst_reg=$(get_reg_num "$dst")
      src_reg=$(get_reg_num "$src")
      ...
      if ((dst_reg_num >= 0 && src_reg_num >= 0)); then
        ...
        [[ -n "$rex" ]] && size=$((size + 1))
        local mod_rm=$(build_mod_rm 3 "$src_reg_num" "$dst_reg_num")
        ...
        local mem_base="${dst#\[}"
        mem_base="${mem_base%\]}"
        local mem_base_num=${regs[${mem_base%%[!a-zA-Z0-9_]*}]:--1}
        local w_flag=$((src_size == 8 ? 1 : 0))
        local rex=$(get_rex_bits "$src_num" "$mem_base_num" "$w_flag")
        ...
        local mem_base_num=${regs[${mem_base%%[!a-zA-Z0-9_]*}]:--1}
        local w_flag=$((dst_size == 8 ? 1 : 0))
        local rex=$(get_rex_bits "$dst_num" "$mem_base_num" "$w_flag")
        ...
        if ((reg_size == 1)); then
          ...
          local mod_rm=$(build_mod_rm 3 0 "$dst_reg_num")
          text_hex+="${rex}c7$(printf "%02x" "$mod_rm")"$(u32le $byte_val)
          local isize=$((6 + ${#rex} / 2))
        ...
        if [[ "$val_is_immediate" -eq 1 ]]; then
          if ((val >= -2147483648 && val <= 2147483647)); then
            local mod_rm=$(build_mod_rm 3 0 "${regs[$reg]}")
            text_hex+="${rex}c7$(printf "%02x" "$mod_rm")"$(u32le $val)
      ...
      case "$cond" in
        ...
        np) cc=0x4b ;;
        *)
          echo "unknown cmov condition $cond" >&2
          return 1
          ;;
      esac
    ...
    elif [[ "$line" =~ ^xor[[:space:]]+([er][a-z]{2}|r[89]|r1[0-5]),[[:space:]]+([er][a-z]{2}|r[89]|r1[0-5])$ && "${BASH_REMATCH[1]}" == "${BASH_REMATCH[2]}" ]]; then
      reg="${BASH_REMATCH[1]}"
      local reg_num=${regs[$reg]:--1}
      local w_flag=$(($(get_reg_size "$reg") == 8 ? 1 : 0))
      local rex=$(get_rex_bits "$reg_num" "$reg_num" "$w_flag")
      local mod_rm=$(build_mod_rm 3 "$reg_num" "$reg_num")
      text_hex+="${rex}31$(printf "%02x" $mod_rm)"
      current_address=$((current_address + 2 + ${#rex} / 2))
    elif [[ "$line" =~ ^push[[:space:]]+([er][a-z]{2}|r[89]|r1[0-5])$ ]]; then
      reg="${BASH_REMATCH[1]}"
      local reg_num=${regs[$reg]:--1}
      local rex=""
      if ((reg_num >= 8)); then
      ...
      op=$((0x50 + (reg_num & 7)))
      text_hex+="${rex}$(printf "%02x" $op)"
      current_address=$((current_address + 1 + ${#rex} / 2))
    elif [[ "$line" =~ ^pop[[:space:]]+([er][a-z]{2}|r[89]|r1[0-5])$ ]]; then
      reg="${BASH_REMATCH[1]}"
      local reg_num=${regs[$reg]:--1}
      local rex=""
      if ((reg_num >= 8)); then
      ...
      dst=$(trim_string "$dst")
      src=$(trim_string "$src")
      if [[ "$dst" =~ ^([er][a-z]{2}|r[89]|r1[0-5])$ && "$src" =~ ^([er][a-z]{2}|r[89]|r1[0-5])$ ]]; then
        # reg, reg
        local reg1="$dst"
        local reg2="$src"
        local w_flag=$(($(get_reg_size "$dst") == 8 ? 1 : 0))
        local rex=$(get_rex_bits "${regs[$src]}" "${regs[$dst]}" "$w_flag")
        local mod_rm=$(build_mod_rm 3 "${regs[$reg2]}" "${regs[$reg1]}")
        local asize=2
        [[ -n "$rex" ]] && asize=$((asize + 1))
        case "$op" in
        ...
        if [[ "$reg" == "rax" ]]; then
          ...
          if ((val >= -128 && val <= 127)); then
            mod_rm=$(build_mod_rm 3 "$op_ext" "${regs[$reg]}")
      ...
      if ((jmp_reg_num >= 0)); then
        ...
        case "$op" in
          jmp)
            ...
            ;;
          je)
            text_hex+="0f8400000000"
            relocations+=("$((current_address + 2)):${lbl}:2:-4")
            current_address=$((current_address + 6))
            ;;
          jne)
            text_hex+="0f8500000000"
            relocations+=("$((current_address + 2)):${lbl}:2:-4")
            current_address=$((current_address + 6))
            ;;
          jg)
            text_hex+="0f8f00000000"
            relocations+=("$((current_address + 2)):${lbl}:2:-4")
            current_address=$((current_address + 6))
            ;;
          jl)
            text_hex+="0f8c00000000"
            relocations+=("$((current_address + 2)):${lbl}:2:-4")
            current_address=$((current_address + 6))
            ;;
          jge)
            text_hex+="0f8d00000000"
            relocations+=("$((current_address + 2)):${lbl}:2:-4")
            current_address=$((current_address + 6))
            ;;
          jle)
            text_hex+="0f8e00000000"
            relocations+=("$((current_address + 2)):${lbl}:2:-4")
            current_address=$((current_address + 6))
            ;;
          ja)
            text_hex+="0f8700000000"
            relocations+=("$((current_address + 2)):${lbl}:2:-4")
            current_address=$((current_address + 6))
            ;;
          jb)
            text_hex+="0f8200000000"
            relocations+=("$((current_address + 2)):${lbl}:2:-4")
            current_address=$((current_address + 6))
            ;;
          jae)
            text_hex+="0f8300000000"
            relocations+=("$((current_address + 2)):${lbl}:2:-4")
            current_address=$((current_address + 6))
            ;;
          jbe)
            text_hex+="0f8600000000"
            relocations+=("$((current_address + 2)):${lbl}:2:-4")
            current_address=$((current_address + 6))
            ;;
          jo)
            text_hex+="0f8000000000"
            relocations+=("$((current_address + 2)):${lbl}:2:-4")
            current_address=$((current_address + 6))
            ;;
          jno)
            text_hex+="0f8100000000"
            relocations+=("$((current_address + 2)):${lbl}:2:-4")
            current_address=$((current_address + 6))
            ;;
          js)
            text_hex+="0f8800000000"
            relocations+=("$((current_address + 2)):${lbl}:2:-4")
            current_address=$((current_address + 6))
            ;;
          jns)
            text_hex+="0f8900000000"
            relocations+=("$((current_address + 2)):${lbl}:2:-4")
            current_address=$((current_address + 6))
            ;;
    ...
    elif [[ "$line" =~ ^(inc|dec|neg|not)[[:space:]]+([er][a-z]{2}|r[89]|r1[0-5])$ ]]; then
      op="${BASH_REMATCH[1]}"
      reg="${BASH_REMATCH[2]}"
      local rex=$(get_rex_for_reg "$reg")
      op_ext=0
      if [[ "$op" == "inc" ]]; then
        mod_rm=$(build_mod_rm 3 0 "${regs[$reg]}")
        text_hex+="${rex}ff$(printf "%02x" $mod_rm)"
      elif [[ "$op" == "dec" ]]; then
        mod_rm=$(build_mod_rm 3 1 "${regs[$reg]}")
        text_hex+="${rex}ff$(printf "%02x" $mod_rm)"
      elif [[ "$op" == "neg" ]]; then
        op_ext=3
        mod_rm=$(build_mod_rm 3 "$op_ext" "${regs[$reg]}")
    ...
    elif [[ "$line" =~ ^call[[:space:]]+([er][a-z]{2}|r[89]|r1[0-5])$ ]]; then
    ...
    elif [[ "$line" =~ ^(mul|div|idiv)[[:space:]]+([er][a-z]{2}|r[89]|r1[0-5])$ ]]; then
      op="${BASH_REMATCH[1]}"
      reg="${BASH_REMATCH[2]}"
      local rex=$(get_rex_for_reg "$reg")
      op_ext=0
      ...
      local lbase="${lmem_content%%[!a-zA-Z0-9_]*}"
      local lrex=$(get_rex_for_mem_operand "$lreg" "$lbase")
      local lhex=$(assemble_mem_operand "$lmem_op" "${regs[$lreg]}" "${lrex}8d")
      text_hex+=$lhex
      current_address=$((current_address + ${#lhex} / 2))
    ...
    elif [[ "$line" =~ ^(shl|shr|sar)[[:space:]]+([er][a-z]{2}|r[89]|r1[0-5]),[[:space:]]+([0-9]+)$ ]]; then
      op="${BASH_REMATCH[1]}"
      reg="${BASH_REMATCH[2]}"
      val="${BASH_REMATCH[3]}"
      local rex=$(get_rex_for_reg "$reg")
      ...
      current_address=$((current_address + 2 + ${#rex} / 2))
    elif [[ "$line" =~ ^test[[:space:]]+([er][a-z]{2}|r[89]|r1[0-5]),[[:space:]]+([0-9]+|0x[0-9a-fA-F]+)$ ]]; then
      reg="${BASH_REMATCH[1]}"
      arg="${BASH_REMATCH[2]}"
      if [[ "$arg" =~ ^0x([0-9a-fA-F]+)$ ]]; then
        val=$((16#${BASH_REMATCH[1]}))
      ...
      current_address=$((current_address + 6 + ${#rex} / 2))
    elif [[ "$line" =~ ^set(e|ne|a|ae|b|be|g|ge|l|le|z|nz|o|no|s|ns)[[:space:]]+([ab][lh]|[cd][lh]|dil|sil|bpl|spl|r[89]b|r1[0-5]b|[er][a-z]{2}|r[89]|r1[0-5])$ ]]; then
      cond="${BASH_REMATCH[1]}"
      dst="${BASH_REMATCH[2]}"
      dst_reg=$(get_reg_num "$dst")
      if ((dst_reg < 0)); then
      ...
      local scl_mod_rm=$(build_mod_rm 3 "$scl_ext" "${regs[$scl_reg]}")
      ...
      local rex=$(get_rex_for_reg "$ireg")
      local imod=$(build_mod_rm 3 "${regs[$ireg]}" "${regs[$ireg]}")
