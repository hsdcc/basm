    ...
    case "$instruction" in
      ...
      add|sub|and|or|cmp)
        ...
        if [[ "$line" =~ ^($op)[[:space:]]+(r[a-z]{2}),[[:space:]]*([0-9]+|0x[0-9a-fA-F]+)$ ]]; then
          ...
          if [[ "$reg" == "rax" ]]; then
            case "$op" in
              add) printf "4881c0"; u32le $val; return 0 ;;
              sub) printf "4881e8"; u32le $val; return 0 ;;
              or)  printf "4881c8"; u32le $val; return 0 ;;
              and) printf "4881e0"; u32le $val; return 0 ;;
              cmp) printf "483d"; u32le $val; return 0 ;;
            esac
          else
            op_ext=0
            case "$op" in
    ...
    case "$instruction" in
      ...
      add|sub|cmp|or|and)
        if [[ "$line" =~ ^($instruction)[[:space:]]+(r[a-z]{2}),[[:space:]]*(.*)$ ]]; then
          ...
          if [[ "$reg" == "rax" ]]; then
            case "$op" in
              add) printf "4881c0"; u32le $val ;;
              sub) printf "4881e8"; u32le $val ;;
              or)  printf "4881c8"; u32le $val ;;
              and) printf "4881e0"; u32le $val ;;
              cmp) printf "483d"; u32le $val ;;
            esac
            return 0
          else
            case "$op" in
